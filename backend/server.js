...
app.use('/Data', express.static(path.join(PROJECT_ROOT, 'public', 'Data')));
...
app.use('/Entamoeba_images', express.static(path.join(PROJECT_ROOT, 'Entamoeba_images')));
...
app.get('/admin-main.js', (req, res) => res.sendFile(path.join(PROJECT_ROOT, 'public', 'admin-main.js')));
app.get('/admin-script.js', (req, res) => res.sendFile(path.join(PROJECT_ROOT, 'public', 'admin-script.js')));
app.get('/admin-style.css', (req, res) => res.sendFile(path.join(PROJECT_ROOT, 'public', 'admin-style.css')));
app.get('/admin.html', (req, res) => res.sendFile(path.join(PROJECT_ROOT, 'public', 'admin.html')));

// Test endpoint to check if protein data is accessible
app.get('/test-protein', (req, res) => {
    const filePath = getDataFilePath(PROJECT_ROOT, 'histolytica', 'protein');
    if (filePath && fs.existsSync(filePath)) {
...
function loadSearchIndex() {
    ...
    if (!fs.existsSync(indexPath)) {
        console.log('⚠ Search index not found. Run: node backend/tools/data/build_search_index.js');
        return;
    }
...
app.get('/search', (req, res) => {
    ...
    const filePath = getDataFilePath(PROJECT_ROOT, organism, dataType);
...
app.get('/admin/api/backups', authenticateAdmin, (req, res) => {
    const dataPath = path.join(PROJECT_ROOT, 'public', 'Data');
    const backupFiles = [];
    try {
...
const DL_OUT_DIR    = path.join(PROJECT_ROOT, 'AmoebaDB_Release68');
const JSON_DIR      = path.join(PROJECT_ROOT, 'AmoebaDB_JSON');
const DL_CONCURRENT = 3;
const DL_RETRY_MAX  = 3;
const DL_RETRY_DELAY = 2000;
const DL_TIMEOUT    = 120_000;
const DL_MAX_REDIRECTS = 10;
...
function dlResolveRedirect(currentUrl, locationHeader) {
    if (!locationHeader) {
...
function dlFetchPage(url, redirectCount = 0) {
    return new Promise((resolve, reject) => {
        const mod = url.startsWith('https') ? https : http;
        const req = mod.get(url, { timeout: DL_TIMEOUT }, (res) => {
            if ([301, 302, 307, 308].includes(res.statusCode)) {
                if (redirectCount >= DL_MAX_REDIRECTS) {
                    return reject(new Error(`Too many redirects for ${url}`));
                }
                const nextUrl = dlResolveRedirect(url, res.headers.location);
                return dlFetchPage(nextUrl, redirectCount + 1).then(resolve, reject);
            }
...
function dlDownloadFile(url, destPath, attempt = 1, redirectCount = 0) {
    return new Promise((resolve, reject) => {
        ...
        const req = mod.get(url, opts, (res) => {
            if ([301, 302, 307, 308].includes(res.statusCode)) {
                ...
                const nextUrl = dlResolveRedirect(url, res.headers.location);
                return dlDownloadFile(nextUrl, destPath, attempt, redirectCount + 1).then(resolve, reject);
            }
        ...
        if (attempt < DL_RETRY_MAX) {
            ...
            return dlDownloadFile(url, destPath, attempt + 1, 0);
        }
        throw err;
    });
}
...
app.use('/raw', express.static(path.join(PROJECT_ROOT, 'AmoebaDB_Release68')));
...
app.get('/api/organisms', (req, res) => {
    try {
        ...
        const curatedDir = path.join(PROJECT_ROOT, 'public', 'Data');
